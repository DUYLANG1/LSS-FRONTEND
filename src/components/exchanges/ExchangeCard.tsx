...
export function ExchangeCard({
  ...
  // Format the date
  const formattedDate = new Date(exchange.createdAt).toLocaleDateString(
    undefined,
    {
      year: "numeric",
      month: "short",
      ...
      <div className="flex flex-col space-y-4">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div>
              <p className="font-medium">
                {isRequester ? "You" : currentUser.name}
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {isRequester ? "Offering" : "Requesting"}:{" "}
                <span className="font-medium">{offeredSkill}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 flex-1">
            ...
            <div>
              <p className="font-medium">
                {isRequester ? otherUser.name : "You"}
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {isRequester ? "Requesting" : "Offering"}:{" "}
                <span className="font-medium">{requestedSkill}</span>
              </p>
            </div>
          </div>
        </div>
